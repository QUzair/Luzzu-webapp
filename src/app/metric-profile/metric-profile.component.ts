...
export class MetricProfileComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //Subscribe to the Current Metric, so we know what metric we are on currently
    this.data.CurrentMetric.subscribe((res) => {
      ...
      //Get Observation for Metric of Specified dataset
    ...
    //Observe the metrics of the Ranked datasets above and below you by two positions
    this.data.currentRankedMetrics.subscribe((res) => {
      console.log('Ranked Dataset metrics')
      console.log(res)
      this.Table = []
      res.map((data) => {
        let found = data.Metrics.map((met) => {
          if (met['Metric-Label'] === this.metric['Metric-Label']) {
            console.log(met)
            let tmp: MetricTableEntry = {
              'Dataset-Label': data.Dataset_PLD,
              'Metric-Value': (met['Observations'][0]['Value-Type']=='Double') ? this.precise(met['Observations'][0]['Value']*100): met['Observations'][0]['Value']
            }
            console.log(tmp)
            this.Table.push(tmp)
          }
  ...
  //Create Donut Chart to show progress of Quality Value
  loadDonut() {
  ...
  //Create Time Graph to show progress over time 
  ...
  //Create Bar Chart to show Number of triples Assessed
  ...
  //Search Description of Metric From facetOptions
  ...
  addTimeData(chart, visData) {
    try{
      ...
      console.log('Observation undefined')
    }
  ...
  FontWeight(entry) {
    if (entry['Dataset-Label']['Dataset-PLD']===this.dataset_label)
        return 'bold';
    else return 'normal'
